...
# Path setup
data_path_red_ts = r'/home/dyno/Downloads/Benefit_of_Distraction-master/Data/RGB_noise_estimates_red'
data_path_green_ts = r'/home/dyno/Downloads/Benefit_of_Distraction-master/Data/RGB_noise_estimates_green'
data_path_blue_ts = r'/home/dyno/Downloads/Benefit_of_Distraction-master/Data/RGB_noise_estimates_blue'

n_examples = 60  # Window size

Xtest = []
Ytest = []

# Read first .mat file for demonstration
filenames = [f for f in os.listdir(data_path_blue_ts) if f.endswith('.mat')]
if filenames:
    filename = filenames[0]
    data_blue = h5py.File(os.path.join(data_path_blue_ts, filename), 'r')
    data_green = h5py.File(os.path.join(data_path_green_ts, filename), 'r')
    data_red = h5py.File(os.path.join(data_path_red_ts, filename), 'r')

    # Flatten all channels
    noise_blue = np.array(data_blue['mask_noise_blue_mean2']).flatten()
    noise_green = np.array(data_green['mask_noise_green_mean2']).flatten()
    noise_red = np.array(data_red['mask_noise_red_mean2']).flatten()
    noisy_signal = np.array(data_red['yptest_sub2']).flatten()
    GT_HR = np.array(data_red['bp_gt_resamp']).flatten()

    # Normalize for plotting
    noise_red /= np.max(np.abs(noise_red)) + 1e-8
    noise_green /= np.max(np.abs(noise_green)) + 1e-8
    noise_blue /= np.max(np.abs(noise_blue)) + 1e-8
    noisy_signal /= np.max(np.abs(noisy_signal)) + 1e-8
    GT_HR /= np.max(np.abs(GT_HR)) + 1e-8

    time = np.arange(len(noise_red))
...
model_path = r'/home/dyno/Downloads/Benefit_of_Distraction-master/get_HR/model_denoising_HR_LSTM.h5'
...
#        print(list(f.keys()))
#        # Access datasets or groups within the file
#        # For example, if it contains a dataset named 'weights'
#        weights = f['weights'][:]
#        # Process 'weights' as needed
...
save_path = r'/home/dyno/Downloads/Benefit_of_Distraction-master/get_HR/model_denoising_HR_LSTM.h5'
...
for filename in os.listdir(data_path_blue_ts): 
    if filename.endswith(".mat"):
        ...
        # Plot all signals in a sample window
        plt.figure(figsize=(12, 7))
        plt.plot(time[:300], noise_red[:300], label='Red Channel Noise')
        plt.plot(time[:300], noise_green[:300], label='Green Channel Noise')
        plt.plot(time[:300], noise_blue[:300], label='Blue Channel Noise')
        plt.plot(time[:300], noisy_signal[:300], label='Noisy Signal')
        plt.plot(time[:300], GT_HR[:300], label='Ground Truth HR', linestyle='--')
        plt.xlabel("Frame Index")
        plt.ylabel("Normalized Value")
        plt.title(f"Sample channels for file: {filename}")
        plt.legend()
        plt.tight_layout()
        plt.show()
    else:
